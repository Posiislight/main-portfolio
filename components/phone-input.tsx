...
const COUNTRIES: Country[] = [
  ...
  { name: "Trinidad & Tobago", dial: "+1", flag: "🇹🇹" },
  ...
  { name: "Other / not listed", dial: "", flag: "🌐" },
]
...
export function PhoneInput() {
  ...
  const filtered = useMemo(() => {
    ...
    return COUNTRIES.filter(
      (c) => c.name.toLowerCase().includes(q) || c.dial.includes(q.startsWith("+") ? q : `+${q.replace(/^\+/, "")}`) || c.dial.includes(q),
    )
  }, [query])
  ...
  const combined = local.trim() ? `${country.dial} ${local.trim()}`.trim() : ""
    ...
    <div ref={rootRef} className="relative">
      <div className="flex gap-2">
        <button
          ...
          className="flex h-11 shrink-0 items-center gap-1.5 border border-input bg-background px-3 font-mono text-sm"
        >
        ...
        <div className="absolute left-0 top-full z-50 mt-2 w-72 border border-emerald-500/40 bg-background shadow-[3px_3px_0_0_rgb(var(--accent-500))]">
          <div className="flex items-center gap-2 border-b px-3 py-2">
            ...
            <input
              ...
              placeholder="Search country or code (e.g. +234)"
              className="w-full bg-transparent text-[16px] outline-none placeholder:text-muted-foreground sm:text-sm"
            />
          </div>
          <ul className="max-h-56 overflow-y-auto py-1" role="listbox">
            ...
            {filtered.length === 0 && (
              <li className="px-3 py-2 text-sm text-muted-foreground">No match. Try the dial code.</li>
            )}
          </ul>
        </div>
      )}
